    ...
    return (
        <div>
                        ...
                        {props.animeList.map((show, index) => (
                            <div className="card mx-2 text-center" key={index} style={{ width: '11rem', height: '30rem', overflow: 'hidden' }}>
                                ...
                                <div className="d-flex flex-column">
                                    ...
                                    <p className="">{show.scoreReview}</p>
                                </div>
                            </div>
                        ))}
                    </Carousel>
            ...
            <div className="text-white text-center">
                <h1 className="text-white mt-3"> {props.animeCount > 0 ? 'Top 15' : ''}</h1>
                    ...
                    props.animeList.length > 0 ? (
                        props.animeList.sort((a, b) => {
                            ...
                            <div className="card mx-2 my-2" style={{ width: '22rem', flexDirection: 'row' }} key={index}>
                                <img src={show.images.jpg.large_image_url} className="card-img-top" alt="wallpaper" style={{ width: '50%',objectFit: 'cover' }}></img>
                                <div className="card-body">
                                    <p className="card-title">{show.title}</p>
                                    <p className="card-text">Score: {show.score}</p>
                                    <p className="card-text">{show.scoreReview}</p>
                                </div>
                            </div>
